    ...
    if len(arg_list) < 2:
        ...
        if arg_list[1] == '-h':
            ...
            if arg_list[1] == '-t':
                ...
                iter = tc_target.comments().__iter__()
                for _ in range(15):
                    try:
                        if 'RICKROLL' in iter.__next__().upper() or 'RICK ROLL' in iter.__next__().upper() or 'RICK ASTLEY' in iter.__next__().upper():
                            details_alert()
                            break
